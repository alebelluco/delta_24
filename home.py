...
#----- OPZIONE DI UTILIZZO 1. CONFRONTO CON DISTINTA DI RIFERIMENTO

if st.toggle('Confronto con distinta'):
    path_1 = st.file_uploader('Caricare file excel')
    if not path_1:
        st.stop()
    df = pd.read_excel(path_1)
    dic_rif = dp.crea_dizionario(df)
    #st.write('prova', prova)

    for i in range(len(df_list)):
        no_mot = dp.rimuovi_motore(df_list[i])
        if len(no_mot) != len(df_list[i]): # se i dataframe di input e output della funzione hanno la stessa lunghezza vuol dire che niente è stato tolto perchè il motore non è stato trovato.
            testo = 'motore ok'
        else:
            testo = 'rilevata assenza motore' 
        desc_moto = no_mot['Testo breve oggetto'].iloc[0]
        modello = desc_moto[4:6]
        dic = dic_rif #---------------------qui sta la differenza rispetto al confronto con dizionario statico
        ...
        st.write(f'SKU: {cod_moto} | {testo}')

#------ OPZIONE DI UTILIZZO 2
else:
    for i in range(len(df_list)):
        no_mot = dp.rimuovi_motore(df_list[i])
        desc_moto = no_mot['Testo breve oggetto'].iloc[0]
